...
func findTotal() {
  ...
  for person in nocList {
    switch person.accessLevel {
      ...
      high += 1
      default:
      print("not between 0-10")
    }
  }
  print("\(low) low level agents, \(med) mid level agents, and \(high) high level agents")
}
...
func printPerson() {
  var arr: [String] = []
  for person in nocList {
    arr.append(person.coverName)
    arr.append("\(person.accessLevel)")
  }
  print(arr)
}
